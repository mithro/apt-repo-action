...
--measure only reports how many bytes the copy would add (fetching the small
index files but no .deb), so keep-history.py can leave room for it.
"""
...
class Copier:
    def __init__(self, site: str, dest: pathlib.Path, measure: bool = False) -> None:
    ...
    def repository(self, path: str, release: bytes) -> None:
        ...
        for rel, size in sorted(debs.items()):
            ...
            self.put(rel, data)
        print(f"carried over repository {path}/ with {len(debs)} packages", file=sys.stderr)
    ...
    def path(self, path: str) -> None:
        if "=" in path:
            ...
            self.put(dest, data)
            print(f"carried over file {src} as {dest}", file=sys.stderr)
            return
        ...
        self.put(path, data)
        print(f"carried over file {path}", file=sys.stderr)
...
def main() -> None:
    ...
    ap.add_argument("--measure", action="store_true",
                    help="print the bytes the copy would add; write nothing")
    ap.add_argument("paths", nargs="+")
    args = ap.parse_args()
    c = Copier(args.site, pathlib.Path(args.dest), args.measure)
    for p in args.paths:
        c.path(p)
    if args.measure:
        print(c.bytes)
    print(f"{c.written} files {'to carry over' if args.measure else 'carried over'}"
          f" ({c.bytes / 1e6:.1f} MB)", file=sys.stderr)
